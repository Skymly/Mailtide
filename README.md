...
Current product is **0.7** (browse a Mailbox's Messages grouped by reply thread). GitHub Release assets are still [v0.1.1](https://github.com/Skymly/Mailtide/releases/tag/v0.1.1) until a human cuts `v0.2.0`.
...
| UI | Unified Inbox view, per-Mailbox browse (reply-thread groups), HTML Message view, compose with optional HTML, local search (`is:unread` / `is:flagged` + text), move a Message to another Mailbox, create a Mailbox, rename a Mailbox, delete a Mailbox |
