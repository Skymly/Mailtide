...
public sealed class DesktopDependencyGuardTests
{
    ...
    {
        var constructors = typeof(BrowseShell).GetConstructors(BindingFlags.Instance | BindingFlags.Public);
    ...
    {
        var browseShell = typeof(BrowseShell);
        ...
        foreach (var member in browseShell.GetMembers(BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly))
        {
            foreach (var type in TypesOf(member))
            {
                Assert.IsFalse(
                    forbidden.Contains(type),
                    $"{member.Name} exposes protocol type {type.Name}");
            }
        }
    }
