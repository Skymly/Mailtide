...
public sealed class MailKitImapAdapterTests
{
    ...
    [TestMethod]
    public async Task Real_IMAP_adapter_clears_Seen_flag()
    {
        await using var server = LoopbackImapServer.Start(
        [
            new SeededMailbox(
        ...
        await client.SetSeenAsync("INBOX", "1");
        Assert.IsTrue((await client.FetchMessagesAsync("INBOX"))[0].IsRead);
        await client.SetUnseenAsync("INBOX", "1");
        var messages = await client.FetchMessagesAsync("INBOX");
        Assert.HasCount(1, messages);
        Assert.IsFalse(messages[0].IsRead);
    }
    ...
    [TestMethod]
    public async Task Real_IMAP_adapter_moves_a_Message_between_Mailboxes()
    {
        await using var server = LoopbackImapServer.Start(
        [
            new SeededMailbox(
                ...
                [
                    new SeededImapMessage(
                        ...
                        Subject: "Bye",
                        From: "bob@example.com",
                        InternalDate: new DateTimeOffset(2026, 8, 8, 12, 0, 0, TimeSpan.Zero),
                        IsRead: false,
                        BodyText: "gone"),
                ]),
            new SeededMailbox(
